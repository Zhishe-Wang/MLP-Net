...
class ROCMetric():
    ...
    def calculateF1Measure(self, preds, labels, score_thresh):
        # for iBin in range(self.bins+1):
        #     score_thresh = (iBin + 0.0) / self.bins
...
class PD_FA():
    ...
    def update(self, preds, labels):
        ...
        for iBin in range(self.bins+1):
            ...
            predits  = np.resize (predits,  (256,256))
            labelss = np.array((labels).cpu()).astype('int64') # P
            labelss = np.resize (labelss , (256,256))
    ...
    def get(self, img_num):
        ...
        # recal = Final_PD
        # prec = 1.0 * Final_PD / (Final_PD + Final_FA * 100000)
